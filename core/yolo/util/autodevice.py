...
from core.yolo.util.checks import check_requirements
...
class GPUInfo:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Warning: Failed to initialize pynvml, GPU stats disabled: {e}")
    ...
    def refresh_stats(self):
        ...
        try:
            ...
            print(f"Warning: Error during device query: {e}")
            self.gpu_stats = []
    ...
    def print_status(self):
        ...
        if not self.gpu_stats:
            print(f"Warning: No GPU stats available.")
            return
        ...
        for gpu in stats:
            ...
            print(f"Info: {gpu.get('index'):<3d} {gpu.get('name', 'N/A'):<{name_len}} {u:>6} {m:>15} {t:>5} {p:>10}")

        print(f"Info: {'-' * len(hdr)}\n")
    ...
    def select_idle_gpu(
        ...
        print(
            f"Info: Searching for {count} idle GPUs with free memory >= {min_memory_fraction * 100:.1f}% and free utilization >= {min_util_fraction * 100:.1f}%..."
        )
        ...
        self.refresh_stats()
        if not self.gpu_stats:
            print(f"Warning: NVML stats unavailable.")
            return []
        ...
        if selected:
            print(f"Info: Selected idle CUDA devices {selected}")
        else:
            print(
                f"Warning: No GPUs met criteria (Free Mem >= {min_memory_fraction * 100:.1f}% and Free Util >= {min_util_fraction * 100:.1f}%)."
            )
